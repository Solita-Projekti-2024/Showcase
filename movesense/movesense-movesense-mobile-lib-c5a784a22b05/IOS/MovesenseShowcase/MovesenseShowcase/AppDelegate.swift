...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func checkAndAddFiletoDocumentsDirectorySoItWillBeVisible() {
        //if no files in document directory add a file so it will be visible
        ...
        do {
            ...
            if files.isEmpty {
                let newFilePath = documentDirectory.appendingPathComponent("NeededForFolderToShow.txt")
                let data = "NeededForFolderToShow".data(using: .utf8)!
                try data.write(to: newFilePath, options: .atomic)
            }
        } catch {
            print("An error occurred while accessing the Documents directory: \(error)")
        }
    }
    ...
    func application(_ application: UIApplication,
        ...
        checkAndAddFiletoDocumentsDirectorySoItWillBeVisible()
